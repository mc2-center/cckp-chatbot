...
### Addendum 3: redirect SQL was sending the literal `{table}` placeholder, unresolved

Live-usage bug, confirmed against a real captured URL: `https://staging.cancercomplexity.synapse.org/Explore/Publications?QueryWrapper0=%7B%22sql%22%3A...%7D`. Decoding `QueryWrapper0` showed `{"sql": "SELECT * FROM {table} WHERE publicationTitle LIKE '%glioma%' OR ...", "additionalFilters": [], "selectedFacets": [], "includeEntityEtag": false, "isConsistent": true}` — the JSON shape and field names (`sql`/`additionalFilters`/`selectedFacets`/`includeEntityEtag`/`isConsistent`) exactly match the portal's real `QueryWrapper0` param, so that part was never wrong. The actual defect: `{table}` was sent to the portal completely unresolved. That placeholder is only ever substituted by the SQL Lambda (`lambda_function.py`'s `sql = sql.replace("{table}", syn_id)`) when the agent calls the `sqlQuery` action — a redirect's `<query>` bypasses the Lambda entirely and goes straight to the portal, which has no idea what `{table}` means, so the resulting query silently returns nothing/wrong results (no error surfaced).

Root cause traced to Addendum 1's condensing pass: the pre-condensed instructions had a full concrete worked-example XML block showing the *resolved* redirect SQL (`"sql": "SELECT * FROM syn21897968 WHERE ..."`); condensing replaced it with a one-line parenthetical ("`{table}` resolved to the table's synId") and no concrete example — evidently not forceful enough, since the model had a vivid `{table}`-literal example immediately above it (the `sqlQuery` tool-call example, where `{table}` is correctly left unresolved for the Lambda to fill in) and pattern-matched to reusing that same string for the redirect too.

Fix: (1) added an explicit, bolded rule right where `<query>` is introduced — "the `sql` must have `{table}` already replaced with the real synId... never send `{table}` literally" — explaining *why* (no Lambda substitution on this path) and what silently breaks if missed; (2) replaced the old parenthetical with a concrete before/after example (`"SELECT * FROM syn21897968 WHERE ..."` vs. `"SELECT * FROM {table} WHERE ..."`) right next to the redirect guidance, so a resolved example is visible at the point of use, not just stated as a rule elsewhere. Caught and fixed a self-introduced slip during this same edit: the first draft's example used `syn21868591` (Publications' synId, copied from the user's bug-report URL) inside a Datasets-context worked example — corrected to `syn21897968` (Datasets) for internal consistency, and separately fixed a dropped 8-space block-scalar indent from the same edit (would have broken the YAML literal block) — caught by the standard re-parse check before finalizing. Instruction length after this change: 10,326 chars — still well under the pre-condensing size.
